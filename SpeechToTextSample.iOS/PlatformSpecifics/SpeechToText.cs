            ...
            _recognitionTask = _speechRecognizer.GetRecognitionTask(_recognitionRequest, (result, error) =>
            {
                ...
                if (result != null)
                {
                    ...
                    if (isNotContinious)
                    {
                        _timer.Invalidate();
                        _timer = null;
                        _timer = NSTimer.CreateRepeatingScheduledTimer(1.5, delegate
                        {
                            DidFinishTalk();
                        });
                    }
